    ...
    for oname in object_list:
        ...
        for bdeg in bdeglist:
            ...
            for file in cam_a_final:
                ...
                ptscolor = np.asarray(downpcd.colors)
                ...
                cam1c = np.expand_dims(np.asarray([0,0,0]),axis=0)
                cam2c = np.expand_dims(np.asarray([0,0,0]),axis=0)
                ptscolor = np.concatenate((ptscolor,cam1c,cam2c),axis=0)
        
                downpcdCam = o3d.geometry.PointCloud()
                downpcdCam.points = o3d.utility.Vector3dVector(pts)
                downpcdCam.colors = o3d.utility.Vector3dVector(ptscolor)
